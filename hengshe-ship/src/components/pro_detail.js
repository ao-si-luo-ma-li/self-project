...
(function() {
	...
	const third_part = ['上海恒舍拍卖有限', '上海恒舍拍卖有限', '上海恒舍拍卖有限'];
	function setGlass() {
		// 商品放大镜效果
		var showproduct = {
			"boxid": "showbox",
	...
	var setDetail = function() {
		...
		if (decodeURI(location.href).split('?')[1] && decodeURI(location.href).split('?')[1].split('&')[1]) {
			storeName = decodeURI(location.href).split('?')[1].split('&')[1] ? unescape(decodeURI(location.href).split('?')[1].split('&')[1].split('=')[1]) : '上海恒舍拍卖有限';
		}
