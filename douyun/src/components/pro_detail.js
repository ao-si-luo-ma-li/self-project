...
(function() {
	...
	const third_part = ['HiPee专营店', '拜安康专营店', '豪横保健器械专营店'];
	function setGlass() {
		// 商品放大镜效果
		var showproduct = {
			"boxid": "showbox",
			"sumid": "showsum",
			"boxw": 390, //宽度,该版本中请把宽高填写成一样
			"boxh": 390, //高度,该版本中请把宽高填写成一样
	...
	var setchose = function(price) {
		...
		if (price) {
			$('#des_money').text(price);
		}
		$('.des_content_tit').html(`暖心价格 <span class="true-c">${$.trim(name)} ${$.trim(ver)} ${$.trim(color)}</span> 产批次不同混合发货`);
	}
	...
	function initShopList() {
		...
		if (arr.length > 0) {
			...
			$('.has-pro .cost').text(prices.reduce((first, second) => first + second));
		}else {
			...
			pro_str += `
				<li class="clearfix">
	                 ...
	                 <div class="right">
	                     ...
	                     <span class="delete" data-name="${_pro['name']}">Delete</span>
	                 </div>
	             </li>
			`;
		}
